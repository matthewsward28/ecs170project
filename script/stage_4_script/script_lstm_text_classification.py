'''
Main orchestration execution script for Stage 4 LSTM text classification tasks
'''
...
from local_code.stage_4_code.Method_LSTM_Text import Method_LSTM_Text
...
if 1:
    ...
    # use method LSTM for Text Classification
    # Note: vocab_size will be dynamically updated by Setting_Train_Test_Load upon dataset loading
    method_obj = Method_LSTM_Text('LSTM-Text', 'gated long short-term memory network for text classification')

    # save results structure tracking independent LSTM folder targets
    result_obj = Result_Saver('saver', '')
    result_obj.result_destination_folder_path = './result/stage_4_result/LSTM_Classification_'
    ...
    # running section
    print('Starting LSTM Text Classification Task')
    ...
    plt.title('LSTM Training Convergence (Text Classification)')
    plt.legend()
    plt.grid(True)
    # Save independent learning curves to the results folder directory
    plt.savefig('./result/stage_4_result/lstm_classification_learning_curve.png') 
    plt.close() 
