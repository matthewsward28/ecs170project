'''
Main orchestration execution script for Stage 4 GRU text generation (Jokes) tasks
'''
...
from local_code.stage_4_code.Method_GRU_Jokes import Method_GRU_Jokes
...
if 1:
    ...
    # use method GRU for Text Generation
    # Note: vocab_size will be dynamically updated by Setting_Train_Test_Load upon dataset loading
    method_obj = Method_GRU_Jokes('GRU-Jokes', 'gated recurrent unit network for word generation')
    ...
    # running section
    print('Starting GRU Text Generation Task')
    ...
    # plotting section
    plt.plot(range(1, len(loss_history) + 1), loss_history, color='teal', label='Training Loss')
    plt.xlabel('Training Epoch')
    plt.ylabel('Loss Value')
    plt.title('GRU Training Convergence (Joke Generation)')
    plt.legend()
    plt.grid(True)
    # Save independent learning curves to the results folder directory
    plt.savefig('./result/stage_4_result/gru_generation_learning_curve.png') 
    plt.close()
