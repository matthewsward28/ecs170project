'''
Main orchestration execution script for Stage 4 LSTM text generation (Jokes) tasks
'''
...
from local_code.stage_4_code.Method_LSTM_Jokes import Method_LSTM_Jokes
...
if 1:
    ...
    # use method LSTM for Text Generation
    # Note: vocab_size will be dynamically updated by Setting_Train_Test_Load upon dataset loading
    method_obj = Method_LSTM_Jokes('LSTM-Jokes', 'gated long short-term memory network for word generation')

    # save results structure tracking independent LSTM folder targets
    result_obj = Result_Saver('saver', '')
    result_obj.result_destination_folder_path = './result/stage_4_result/LSTM_Generation_'
    ...
    # running section
    print('Starting LSTM Text Generation Task')
    setting_obj.prepare(data_obj, method_obj, result_obj, evaluate_obj)
    
    # this handles loading, text normalization, sliding-window tokenization, and optimized mini-batch training loops
    metrics, loss_history = setting_obj.load_run_save_evaluate()

    # plotting section
    plt.plot(range(1, len(loss_history) + 1), loss_history, color='orange', label='Training Loss')
    plt.xlabel('Training Epoch')
    plt.ylabel('Loss Value')
    plt.title('LSTM Training Convergence (Joke Generation)')
    plt.legend()
    plt.grid(True)
    # Save independent learning curves to the results folder directory
    plt.savefig('./result/stage_4_result/lstm_generation_learning_curve.png') 
    plt.close()
